...
namespace SimpleGameEngine::Projection
{
	...
	void OrthoProjectionBox::update(const Math::Mat4 & lightViewMatrix, const Math::Vec3 & eyePosition, const Math::Vec3 & eyeRotation)
	{
		...
		Vec3 toNear = forward * m_nearPlane;
		Vec3 centerFar = toFar + eyePosition;
		Vec3 centerNear = toNear + eyePosition;

		std::vector<Vec4> points = calculateFrustumVertices(lightViewMatrix, rotation, forward, centerNear, centerFar);
	...
	Math::Vec3 OrthoProjectionBox::getLightSpaceCenter() const
	{
	...
	OrthoProjectionBox OrthoProjectionBox::GenerateOrthoProjectionBox(float fov, float nearPlane, float aspectRatio)
	{
		...
		return OrthoProjectionBox(0, 0, 0, 0, 0, 0, nearWidth, nearHeight, farWidth, farHeight, nearPlane);
	}



	std::vector<Math::Vec4> OrthoProjectionBox::calculateFrustumVertices(const Math::Mat4 & lightViewMatrix,
																		 ...
																		 const Math::Vec3 & centerFar) const
	{
		...
		frustumVertices.push_back(calculateLightSpaceFrustumCorner(lightViewMatrix, farTop, right, m_farWidth));
		frustumVertices.push_back(calculateLightSpaceFrustumCorner(lightViewMatrix, farTop, left, m_farWidth));
		
		frustumVertices.push_back(calculateLightSpaceFrustumCorner(lightViewMatrix, farBot, right, m_farWidth));
		frustumVertices.push_back(calculateLightSpaceFrustumCorner(lightViewMatrix, farBot, left, m_farWidth));
		
		frustumVertices.push_back(calculateLightSpaceFrustumCorner(lightViewMatrix, nearTop, right, m_nearWidth));
		frustumVertices.push_back(calculateLightSpaceFrustumCorner(lightViewMatrix, nearTop, left, m_nearWidth));
		
		frustumVertices.push_back(calculateLightSpaceFrustumCorner(lightViewMatrix, nearBot, right, m_nearWidth));
		frustumVertices.push_back(calculateLightSpaceFrustumCorner(lightViewMatrix, nearBot, left, m_nearWidth));

		return frustumVertices;
	}

	Math::Vec4 OrthoProjectionBox::calculateLightSpaceFrustumCorner(const Math::Mat4 & lightViewMatrix, 
																	const Math::Vec3 & startPoint, 
																	const Math::Vec3 & direction, 
																	float width) const
	{
		Vec3 point = startPoint + (direction * width);
		return lightViewMatrix * Vec4(point.x, point.y, point.z, 1);
	}
