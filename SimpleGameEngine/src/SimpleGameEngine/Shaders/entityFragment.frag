...
void main()
{
	...
	for (int i = 0; i < 5; i++)
	{
		...
		float diffuse_amount = max(dot(n_to_light, vNormal), 0.0);
		total_diffuse += (diffuse_amount * uLightColors[i]) / attenuation_factor;

		// Calculate specular portion of color
		float specular_amount = pow(max(dot(vToEye, vLightReflections[i]), 0.0), uSpecularHighlight);
