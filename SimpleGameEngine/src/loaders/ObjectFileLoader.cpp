...
Model* ObjectFileLoader::loadOBJ(const char* filepath, const char* texturefilepath)
{
	...
	*/
	...
	MessageHandler::printMessage(std::string(filepath) + " has been loaded!\n");
...
Material* ObjectFileLoader::loadMaterial(std::string filepath, const char* texturepath)
{
	...
	if (materialFile.is_open() == false)
		MessageHandler::printMessage("ERROR: Cannot open " + path +"!\n");
	...
	MessageHandler::printMessage(path + " has been loaded!\n");
