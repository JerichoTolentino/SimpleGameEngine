...
/*
TODO:
9999. FIX RAY CASTING INACCURACY!
	- IN PROGRESS:
		- Figure out way to draw a ray from last left-click location
			- Currently just verifying camera pos (ray start pos), ray dir, and ray end pos.
0.1. Have camera enter free-walk mode/strafe mode depending on character status (if mouse button clicked or smth...)
	- Add fields to Camera class
		- bool lockedHorizontal
		- bool lockedVertical
	- When rotating camera around entity (in controller.cpp), check if either is true first
1. Write loaders
	a. Implement support for texture seams (2+ texture-coords, 1 vert)
2. Implement animation engine using Blender
*/

/*
BUGS:
0. Fix bug with BoundingBox Class imported from 4490 project!!!!!
	- Solved; bug due to circular includes(?)
1. Fix random bug in Skybox.cpp (//TODO: ~ln270ish)
2. Fix objects stretching near edge of window (aspect ratio? view matrix? projection matrix?)
3. FIX RAY CASTING/INTERSECTION TEST FOR RAY CASTER
	- Probably a problem with conversion from mouse coords to world coords
*/

/*
IDEAS:
1. Implement half-way vector for efficiency (rather than reflection vector in fragment shader)
	- Doesn't change based on view angle 
*/

/*
OPTIMIZE:
1. Controller.cpp
	- Deciding what rotY the entity has! (Too many if statements..)
*/